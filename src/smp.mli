...
val start_smp : State.config -> State.enc_data -> State.smp_state -> ?question:string -> string -> (State.smp_state * Cstruct.t option) or_error
...
val handle_secret : State.config -> State.enc_data -> State.smp_state -> string -> (State.smp_state * Cstruct.t option) or_error
